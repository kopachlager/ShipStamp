  ...
  try {
    ...
    return (
      <ProjectState
        ...
        copy="The project route does not contain a valid GitHub repository identifier."
      />
    );
  }
  ...
  try {
    ...
    if (error instanceof ContractNotConfiguredError) {
      ...
      failure = {
        ...
        copy: "The project timeline could not be loaded safely.",
      };
    }
  }

  if (failure)
    return <ProjectState title={failure.title} copy={failure.copy} />;

  return (
    <main className="mx-auto max-w-6xl px-5 py-12 sm:px-8 sm:py-20">
      <div className="mb-10 border-b border-border pb-8">
        <p className="technical-label text-primary">
          Chronological onchain manifest
        </p>
        <h1 className="display-title mt-4 break-words text-6xl sm:text-7xl">
          {repository}
        </h1>
